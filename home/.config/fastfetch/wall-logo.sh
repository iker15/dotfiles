#!/usr/bin/env bash
# Recorte cuadrado del fondo actual de Caelestia para el logo de fastfetch, con marco claro del tema.
# Solo se regenera si cambia el fondo o el esquema de colores.
src=$(readlink -f ~/.local/state/caelestia/wallpaper/current)
scheme=~/.local/state/caelestia/dark/scheme.json
...
if [[ ! -f $out || "$(cat "$stamp" 2>/dev/null)" != "$key" ]]; then
    colour=$(grep -oP '"onSurface":\s*"\K[0-9a-fA-F]{6}' "$scheme" 2>/dev/null | head -1)
    magick "$src" -resize 480x480^ -gravity center -extent 480x480 \
        -bordercolor "#${colour:-cccccc}" -border 5 "$out" && echo "$key" > "$stamp"
fi
