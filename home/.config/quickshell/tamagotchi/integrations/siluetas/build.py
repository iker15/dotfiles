...
CFG = {
    "ditto": dict(size=1.3, body=[0, 2, 4, 6, 7], color=0, ink=[1, 3, 5],
                  marks=[dict(k=[8], face=True)],
    ...
    "pikachu": dict(size=1.05, body=[0, 3, 5, 6, 8], color=0, ink=[1],
                    marks=[dict(k=[2, 7])],
                    eyes=dict(k=[1], at=[(0.28, 0.63), (0.72, 0.63)], shine=True)),
    ...
    "gengar": dict(size=1.45, body=[0, 1, 2, 5], color=2, ink=[4, 8],
                   ...
                   eyes=dict(k=[4, 8], sclera=[7], at=[(0.33, 0.46), (0.56, 0.51)], tilt=0.5)),
    "snorlax": dict(size=1.3, body=[1, 4, 7], color=1, ink=[2, 8],
                    marks=[dict(k=[0, 5]), dict(k=[3, 6])],
                    eyes=dict(k=[2, 8], at=[(0.43, 0.37), (0.57, 0.37)], lid=0.92, d=0.14)),
    ...
    "calcifer": dict(size=1.12, body=[0], color=0, ink=[3, 5, 4],
                     marks=[dict(k=[2, 8, 7])],
                     eyes=dict(k=[3, 5], sclera=[1, 6], at=[(0.30, 0.75), (0.70, 0.75)])),
...
def build(sid, c):
    ...
    if asd and bsd:
        eyedef["white"] = hexc(cols[e["sclera"][0]])
    ...
    def add(mask, colr, face, thin=False):
        mask = mask & ~eyemask
        if not thin:
        ...
        for comp in rings(keep):
            ...
            if rr:
                polys.append(rr)
        if polys:
            marks.append({"c": colr, "f": 1 if face else 0, "p": polys})

    for mk in c["marks"]:
        add(isin(mk["k"]), hexc(cols[mk["k"][0]]), mk.get("face", False))
    if c["ink"]:
        ink = isin(c["ink"])
        core = ink & (edge > band)
        ...
        add(ndi.binary_dilation(grown & ~edgey, iterations=1) & m, hexc(cols[c["ink"][0]]), True, thin=True)
        add(edgey, hexc(cols[c["ink"][0]]), False, thin=True)
    return {"color": hexc(cols[c["color"]]), "eyes": eyedef, "marks": marks}
...
def manual():
    ...
    py = lambda j: -1 + 2 * j / 8
    res["mcslime"] = {"color": "#79c35c", "eyes": {"x": 0.0, "y": round(py(3), 3), "g": 0.5, "d": 0.25, "ink": "#1c3a12", "round": 0},
                      "marks": [{"c": "#5ea244", "f": 0, "p": [q(px(1), py(1), px(7), py(7))]},
                                {"c": "#1c3a12", "f": 1, "p": [q(px(4), py(5), px(5), py(6))]}]}
    # Blinky (sprite de la recreativa: ojos blancos de 4×5 con la pupila azul de 2×2)
    res["blinky"] = {"color": "#ff0000", "eyes": {"x": 0.0, "y": -0.29, "g": 0.43, "d": 0.3, "ink": "#2121de", "white": "#ffffff", "ws": 1.9},
                     "marks": []}
    # Baymax: los dos puntos negros unidos por una raya
    res["baymax"] = {"color": "#f4f4f2", "eyes": {"x": 0.0, "y": 0.2, "g": 0.4, "d": 0.2, "ink": "#141414"},
                     "marks": [{"c": "#141414", "f": 1, "p": [[flat(orient(np.array([[-0.4, 0.18], [0.4, 0.18], [0.4, 0.23], [-0.4, 0.23]], float), True))]]}]}
    return res
...
def main():
    ...
    for sid, s in SIZES.items():
        data[sid]["size"] = s
    order = ["ditto", "nook", "pikachu", "kirby", "jigglypuff", "gengar", "snorlax", "boo", "totoro", "calcifer", "mcslime", "blinky", "baymax", "isabelle"]
    lines = [".pragma library", "",
             ...
             "//   eyes  x, y centro entre los ojos · g media separación · d diámetro · ink/white/ws/lid/tilt",
             "//   marks [{c color, f 1 = de la cara (boca, líneas; también en su color), p [[anillo exterior, agujeros…]]}]",
             "const shapes = {"]
    for sid in order:
        d = data[sid]
    ...
    for sid in order:
        d = data[sid]
        print(sid, d["color"], "ojos", d["eyes"], "detalles", len(d["marks"]), sum(len(p) for mk in d["marks"] for p in mk["p"]))
